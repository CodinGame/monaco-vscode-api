...
registerExtHostProvider('languageFeatures', {
  ...
  provide: (accessor, mainContext, { extHostFileSystemInfo, extHostLogService, extHostApiDeprecationService, extHostTelemetry, extHostDocuments, extHostCommands, extHostDocumentsAndEditors }) => {
    const uriTransformerService = accessor.get(IURITransformerService)
    const rpcProtocol = accessor.get(IExtHostRpcService)

    const extHostDiagnostics = rpcProtocol.set(ExtHostContext.ExtHostDiagnostics, new ExtHostDiagnostics(mainContext, extHostLogService!, extHostFileSystemInfo!, extHostDocumentsAndEditors!))
