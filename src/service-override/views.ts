...
function isEditorPartVisible(): boolean {
  const container = (
    StandaloneServices.get(IEditorGroupsService).mainPart as EditorPart
  ).getContainer()
...
type InitializationStateTransformer = (
...
onRenderWorkbench(async (accessor) => {
  ...
  function getInitialEditorsState(): IInitialEditorsState | undefined {
    ...
    if (
      ((defaultLayout?.editors != null && defaultLayout.editors.length > 0) ||
  ...
  function getDefaultLayoutViews(
  ...
  function shouldRestoreEditors(
    ...
    const forceRestoreEditors =
      configurationService.getValue<string>('window.restoreWindows') === 'preserve'
    return !!forceRestoreEditors || initialEditorsState === undefined
  }

  async function resolveEditorsToOpen(
    ...
    if (initialEditorsState != null) {
      ...
      const filesToMerge = coalesce(
        await pathsToEditors(initialEditorsState.filesToMerge, fileService, logService)
      )
      if (
      ...
      const filesToDiff = coalesce(
        await pathsToEditors(initialEditorsState.filesToDiff, fileService, logService)
      )
  ...
  function getDefaultViewContainer(location: ViewContainerLocation) {
    return (
      viewDescriptorService.getDefaultViewContainer(location) ??
  ...
  function initLayoutState() {
    if (layoutService.isVisible(Parts.SIDEBAR_PART)) {
      ...
      if (
        ...
        viewContainerToRestore = storageService.get(
          SidebarPart.activeViewletSettingsKey,
          StorageScope.WORKSPACE,
          getDefaultViewContainer(ViewContainerLocation.Sidebar)?.id
        )
      } else {
    ...
    if (layoutService.isVisible(Parts.PANEL_PART)) {
      const viewContainerToRestore = storageService.get(
        ...
        StorageScope.WORKSPACE,
        getDefaultViewContainer(ViewContainerLocation.Panel)?.id
      )
  ...
  for (const { id, role, classes, options, getPosition, onDidChangePosition } of [
    ...
    },
    {
      ...
      getPosition: () =>
        layoutService.getSideBarPosition() === Position.LEFT ? Position.RIGHT : Position.LEFT,
      onDidChangePosition: layoutService.onDidChangeSideBarPosition
    },
    ...
    (async () => {
      ...
      layoutRestoredPromises.push(
        Promise.all([
          ...
          editorGroupService.mainPart.whenRestored.finally(() =>
  ...
  layoutReadyPromises.push(
    (async () => {
      ...
      if (viewlet == null) {
        await paneCompositePartService.openPaneComposite(
          getDefaultViewContainer(ViewContainerLocation.Sidebar)?.id,
          ViewContainerLocation.Sidebar
  ...
  layoutReadyPromises.push(
    (async () => {
      ...
      const panel = await paneCompositePartService.openPaneComposite(
        initialLayoutState.views.containerToRestore.panel,
...
function getServiceOverride(
  openEditorFallback?: OpenEditor,
  _webviewIframeAlternateDomains?: string,
  initializationState?: InitializationStateTransformer
): IEditorOverrideServices
function getServiceOverride(
  openEditorFallback?: OpenEditor,
  _webviewIframeAlternateDomains?: string,
  initializationStateOrRestoreEditors?: boolean | InitializationStateTransformer
): IEditorOverrideServices {
