...
import { BASE_DIR, EXTENSIONS, pkg, external, VSCODE_SRC_DIR } from './config.js'
  ...
  return subpackagePlugin({
      ...
      const baseManifest: Manifest = {
        ...
        dependencies: Object.fromEntries(
          externalDependencies
            ...
            .filter((d) => Array.from(d.importers).some((importer) => !importer.endsWith('.d.ts')))
            .map((d) => {
    ...
    async finalize(subpackages, getModule) {
      ...
      function createTree(moduleId: string, seen: Set<String> = new Set<string>()): Node {
        if (seen.has(moduleId)) {
          return {
            ...
            importers: [
              {
                module: '<break cyclic dependency>',
                importers: []
              }
            ]
          }
        ...
        return {
          module: moduleId,
          subpackage: subpackages.find((p) => p.modules.has(moduleId))?.name,
          importers: Array.from(module!.importers).map((module) => createTree(module, seen))
        }
      }
      ...
      for (const notAllowedDependency of notAllowedDependencies) {
        ...
        // TODO, delete branch that aren't related to the main package
        const tree = importers.map((module) => createTree(module))
        ...
        function printTree(node: Node, prefix: string = '', isLast: boolean = true): string[] {
          ...
          const content = [`${prefix}${connector}${module}`]
        ...
        this.warn(`Not allowed dependency ${notAllowedDependency.name}:\n${renderedTree}`)
      }
